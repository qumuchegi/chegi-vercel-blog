...
const ArticlContentRoue: RouteItem = {
  ...
  component: (
    <Loadable
      LazyComponent={lazy(
        () => import(/*Article content */ "@/pages/ArticleContent")
      )}
    />
  ),
};
export const RenderArticleWithNotionId: RouteItem = {
  ...
  component: (
    <Loadable
      LazyComponent={lazy(
        () => import(/*Article content */ "@/pages/ArticleContentWithId")
      )}
    />
  ),
};
const ArticleRoute: RouteItem = {
  ...
  component: (
    <Loadable
      LazyComponent={lazy(
        () => import(/*  ArticleList */ "@/pages/ArticleList")
      )}
      childRoutes={[ArticlContentRoue]}
      childrenLayout={window.screen.availWidth < 768 ? "verticle" : "horzontal"}
    />
  ),
};
...
export const useNaviToArticleDetail = () => {
  const history = useHistory();
  return useCallback(
    (params: { tabId: string; articleId: string }) => {
      history.push(
        `/home/article-list/${params?.tabId}/article-content/${params?.articleId}`
      );
    },
    [history]
  );
