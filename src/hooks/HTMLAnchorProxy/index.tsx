...
export const useRewriteAnchors = (
  ...
  rewriteAnchorAttributes: Record<
    string,
    (oldValue: any, AnchorNode: HTMLAnchorElement) => any
  >,
  ...
  const rewrite = useCallback(() => {
    ...
    for (let i = 0; i < anchorNodes.length; i++) {
      if (onClick) {
        anchorNodes[i].addEventListener("click", () =>
          onClick(anchorNodes[i].getAttribute("href") ?? "", anchorNodes[i])
        );
      }
      Object.entries(rewriteAnchorAttributes).forEach(([k, vCreator]) => {
        anchorNodes[i].setAttribute(
          k,
          vCreator(anchorNodes[i].getAttribute(k) ?? "", anchorNodes[i])
        );
      });
    }
