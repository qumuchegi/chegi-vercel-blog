...
import {
  useNaviToArticleDetail,
  useNaviToArticleList,
} from "@/routes/category/article";
...
export default function Home() {
  ...
  const changeSelectedTab = useStore(
    (state) => state.uiState.actions.changeSelectedTab
  );
  ...
  const { open: openBottomSeet, close: closeBottomSheet } = useBottomSheet();
  const tabNames = useMemo(
    () => (tabArticles ? Object.keys(tabArticles) : []),
    [tabArticles]
  );
  ...
  return (
    <div className={styles.container} ref={headerRef}>
      <div
        className={combineClassNames(
          styles.tabContainer,
          showArticleTitle ? styles.up : styles.down
        )}
      >
        {tabs}
